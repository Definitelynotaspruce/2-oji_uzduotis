...
void skaitymasfailo(std::deque<Studentas> &a, char &mediana, std::string failopavadinimas);

void ivedinejimas(std::deque<Studentas> &a, char &mediana);

// cia ivedinejimo pradzia
std::deque<Studentas> nuskaitymas(int &n, char &failai);

bool alfa(Studentas &a, Studentas &b);

// duomenu isvedimo funkcija/
void isvedimas(std::deque<Studentas> &a, int &n, std::string failovardas, char &sortinimas);